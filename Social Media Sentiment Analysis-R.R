...
google_client_id <- "801414439690-1ecvm6tkirt5k2d75kitfgqag7fc5uoe.apps.googleusercontent.com"

google_client_secret <- "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"
...
.hero-section {
  ...
  background: linear-gradient(
    ...
    url('https://files.oaiusercontent.com/file-L7TTpCeurYgamL27nySWWh?se=2025-01-02T12%3A50%3A18Z&sp=r&sv=2024-08-04&sr=b&rscc=max-age%3D604800%2C%20immutable%2C%20private&rscd=attachment%3B%20filename%3Dd0ca0a06-e766-4720-814f-def79348141d.webp&sig=C%2BrHYRXxSxgxPwKwEj3iEqcEw152DW7ENZAI7l%2BSEIs%3D');
  background-size: cover;
  background-position: center;
  height: 105vh; /* Slightly reduced height */
  color: white;
  text-align: center;
...
.user-info-container .box h4 {
    ...
    tabPanel(
      ...
      div(
        class = "hero-section",
        h1("Understanding Social Media Sentiments"),
        p(
          "Discuss how data-driven models help understand a person’s sentiment and emotions based on social media activity."
        ),
      ...
      div(
        ...
        h3("Our Mission"),
        p(
          "We use technology and data science to analyze, predict, and visualize people's sentiments and emotions based on social media activity. Our platform offers valuable insights for brands, marketers, and social analysts."
        ),
        div(
          ...
          div(
            ...
            p("Use ML models to predict sentiment and analyze emotional trends.")
          ),
          div(
            class = "feature-box",
            tags$i(
             ...
             fluidRow(
               ...
               column(12, 
                      div(class = "login-instruction-container",
                          ...
                          p("Use your Google account to sign in and start exploring the sentiment prediction models."),
...
server <- function(input, output, session) {
  ...
  observeEvent(input$login_btn, {
    ...
    appendTab(inputId = "main_navbar", tabPanel("Prediction",
                                                ...
                                                fluidRow(
                                                  ...
                                                  column(12,
                                                         div(class = "instruction-container",
                                                             h2("How to predict sentiment scores and analyze emotional risk based on social media data."),
                                                             p("Follow these steps to predict sentiment scores based on social media activity using the provided parameters."),
                                                  ...
                                                  column(12, 
                                                         div(class = "prediction-steps-container",
                                                             ...
                                                             div(class = "step-box",
                                                                 ...
                                                                 div(class = "step-content",
                                                                     h4("Step 3: Predict Sentiment"),
                                                                     p("Click the 'Predict Sentiment' button to calculate the sentiment.")
                                                                 )
                                                             ),
                                                             div(class = "step-box",
                                                                 ...
                                                                 div(class = "step-content",
                                                                     ...
                                                                     p("The sentiment will be displayed below in real-time based on social media activity.")
                                                                 )
                                                             )
                                                         )
                                                  ),
                                                  ...
                                                  column(12, 
                                                         box(
                                                           ...
                                                           actionButton("predict", "Predict Sentiment", icon = icon("calculator"), class = "btn-predict")
                                                         )
                                                  ),
  ...
  observeEvent(input$file, {
    ...
    data_cleaned <- raw_data[, !(names(raw_data) %in% c("id"))]
    data_cleaned$text <- as.factor(data_cleaned$text)
    data_cleaned$platform <- as.factor(data_cleaned$platform)
    data_cleaned$country <- as.factor(data_cleaned$country)
    data_cleaned$sentiment <- as.factor(data_cleaned$sentiment)
    ...
    set.seed(42)
    trainIndex <- createDataPartition(data_cleaned$sentiment, p = 0.8, list = FALSE)
    train_data <- data_cleaned[trainIndex,]
    
    rf_model <- randomForest(sentiment ~ ., data = train_data, ntree = 100, importance = TRUE)
  ...
  observeEvent(input$predict, {
    ...
    output$prediction_result <- renderUI({
      HTML(sprintf("<span style='color:%s;'>Predicted Sentiment: %s</span>", color, prediction))
    })
  })
